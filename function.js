...
function getWIBTime() {
    const now = new Date();
    const utcOffset = now.getTimezoneOffset() * 60000; // Offset UTC dalam milidetik
    const jakartaOffset = 7 * 60 * 60000; // Offset Jakarta (WIB) adalah UTC+7
    const wibTime = new Date(now.getTime() + utcOffset + jakartaOffset);
    
    // Format waktu menjadi YYYY-MM-DD HH:mm:ss
    const formattedTime = wibTime.toISOString().slice(0, 19).replace('T', ' ');
